...
### Permission Codes (Tasks)

| Code | Description |
|------|-------------|
| `tasks:read` | View tasks list and detail pages |
| `tasks:write` | Create, edit, mark complete tasks |
| `tasks:delete` | Delete tasks (single and bulk) |
...
4. Contacts, Companies, and Deals created (from prior V2 testing guides) for relation linking in the "Related Information" section
...
- Tasks use **soft delete** — deleted tasks are hidden from the UI but retained in the database
- The **Assigned To** dropdown loads organisation members via API — ensure at least one member exists
