...
const INITIAL_NEW_FIELD = {
  ...
  readonly: false,
  conditional: null as { show_if: { field: string; operator: string; value: string } } | null,
};

export default function FormLayoutEditor({
  entityType,
  entityLabel,
  entityLabelPlural,
  backUrl,
  ...
  const [newField, setNewField] = useState({ ...INITIAL_NEW_FIELD });
  ...
  const loadFormData = async () => {
    try {
      const schema = await formsV2Api.getFormSchema(entityType, 'create');
  ...
  const handleCreateField = () => {
    ...
    if (needsOptions(newField.field_type) && newField.options.length === 0) {
    ...
    const inlineField: InlineFieldDefinition = {
      ...
      field_type: newField.field_type as InlineFieldDefinition['field_type'],
  ...
  const handleAddOption = () => {
    setNewField({
      ...newField,
      options: [...newField.options, { value: `option_${Date.now()}`, label: 'New Option' }]
    });
  };
  ...
  const handleEditFieldProperties = (sectionIndex: number, fieldIndex: number) => {
    ...
    setEditingField({ sectionIndex, fieldIndex, field: { ...field } });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: formsV2QueryKeys.formSchema(entityType, 'create')
      });
      toast.success(`${entityLabel} form layout saved successfully!`);
    } catch {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
              <div className="text-xs text-muted-foreground hidden lg:block">
                {sections.length} section{sections.length !== 1 ? 's' : ''} &bull; {sections.reduce((acc, s) => acc + s.fields.length, 0)} field{sections.reduce((acc, s) => acc + s.fields.length, 0) !== 1 ? 's' : ''}
              </div>
              <Button
                onClick={() => { setTargetSectionIndex(0); setShowNewFieldDialog(true); }}
                size="sm"
              ...
              <Button onClick={handleAddSection} variant="outline" size="sm" className="h-8 text-xs sm:text-sm">
                <Plus className="h-3.5 w-3.5 mr-1 sm:mr-1.5" />
                <span className="hidden sm:inline">Add Section</span>
                <span className="sm:hidden">Section</span>
              </Button>
              <Button variant="outline" onClick={() => router.push(backUrl)} size="sm" className="h-8 hidden md:inline-flex">
                <Eye className="h-4 w-4 mr-1.5" />
                Preview
              </Button>
              <Button onClick={handleSave} disabled={isSaving} size="sm" className="h-8 text-xs sm:text-sm">
                                ...
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                      {field.field_type} &bull; {field.width === 'full' ? 'Full Width' : field.width === 'half' ? '50%' : field.width}
                                    </div>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    <Button variant="ghost" size="sm" onClick={() => handleEditFieldProperties(sectionIndex, fieldIndex)} className="text-muted-foreground hover:text-primary h-7 w-7 p-0">
                                      <Settings className="h-3.5 w-3.5" />
                                    </Button>
                                    <Button variant="ghost" size="sm" onClick={() => handleRemoveField(sectionIndex, fieldIndex)} className="text-muted-foreground hover:text-destructive h-7 w-7 p-0">
                ...
                <div className="flex items-center justify-between mt-3 pt-2 border-t border-border">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => { setTargetSectionIndex(sectionIndex); setShowNewFieldDialog(true); }}
              ...
              >
                <div className="p-4 border-b sticky top-0 bg-white z-10">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setShowNewFieldDialog(false)} className="h-8 w-8 p-0 flex-shrink-0">
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
                ...
                <div className="p-4 space-y-3">
                    ...
                    <div>
                      ...
                      >
                        {sections.map((section, idx) => (
                          <option key={section.id} value={idx}>{section.title} ({section.fields.length} fields)</option>
                        ))}
                      </select>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input value={newField.label} onChange={(e) => setNewField({ ...newField, label: e.target.value })} placeholder="e.g., Company Name" className="h-8 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs mb-1 block">Field Name</Label>
                      <Input value={newField.name} onChange={(e) => setNewField({ ...newField, name: e.target.value })} placeholder="e.g., company_name (auto-generated)" className="h-8 text-sm" />
                  ...
                  <div>
                    ...
                    <select value={newField.field_type} onChange={(e) => setNewField({ ...newField, field_type: e.target.value })} className="w-full rounded-md border border-input bg-background px-2.5 py-1.5 text-sm h-8">
                      ...
                      <div className="space-y-1.5">
                        {newField.options.map((option, index) => (
                          <div key={index} className="flex gap-1.5">
                            <Input value={option.label} onChange={(e) => handleUpdateOption(index, { label: e.target.value })} placeholder="Option label" className="h-8 text-sm flex-1" />
                            <Input value={option.value} onChange={(e) => handleUpdateOption(index, { value: e.target.value })} placeholder="Value" className="h-8 text-sm flex-1" />
                            <Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveOption(index)} className="h-8 w-8 p-0 flex-shrink-0">
                              <Trash2 className="h-3.5 w-3.5" />
                            </Button>
                          </div>
                        ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input value={newField.placeholder} onChange={(e) => setNewField({ ...newField, placeholder: e.target.value })} placeholder="Enter placeholder text" className="h-8 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs mb-1 block">Width</Label>
                      <select value={newField.width} onChange={(e) => setNewField({ ...newField, width: e.target.value as typeof newField.width })} className="w-full rounded-md border border-input bg-background px-2.5 py-1.5 text-sm h-8">
                  ...
                  <div>
                    ...
                    <Textarea value={newField.help_text} onChange={(e) => setNewField({ ...newField, help_text: e.target.value })} placeholder="Help text to guide users" rows={2} className="text-sm resize-none" />
                  </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <label className="flex items-center space-x-1.5 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setNewField({ ...newField, conditional: { show_if: { field: '', operator: 'equals', value: '' } } });
                            } else {
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setNewField({ ...newField, conditional: { ...newField.conditional!, show_if: { ...newField.conditional!.show_if, field: e.target.value } } })}
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setNewField({ ...newField, conditional: { ...newField.conditional!, show_if: { ...newField.conditional!.show_if, operator: e.target.value } } })}
                              className="w-full rounded-md border border-input bg-white px-2 py-1 text-xs h-7"
                            >
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setNewField({ ...newField, conditional: { ...newField.conditional!, show_if: { ...newField.conditional!.show_if, value: e.target.value } } })}
                              placeholder="Value..."
                  ...
                  <div className="flex items-center gap-3 flex-wrap pt-3 border-t">
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={newField.is_required} onChange={(e) => setNewField({ ...newField, is_required: e.target.checked })} className="rounded border-gray-300 h-3.5 w-3.5" />
                      <span className="text-xs">Required</span>
                    </label>
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={newField.is_unique} onChange={(e) => setNewField({ ...newField, is_unique: e.target.checked })} className="rounded border-gray-300 h-3.5 w-3.5" />
                      <span className="text-xs">Unique</span>
                    </label>
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={newField.is_searchable} onChange={(e) => setNewField({ ...newField, is_searchable: e.target.checked })} className="rounded border-gray-300 h-3.5 w-3.5" />
                      <span className="text-xs">Searchable</span>
                    </label>
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={newField.readonly} onChange={(e) => setNewField({ ...newField, readonly: e.target.checked })} className="rounded border-gray-300 h-3.5 w-3.5" />
                ...
                <div className="p-3 border-t flex justify-end gap-2 sticky bottom-0 bg-white">
                  <Button variant="outline" onClick={() => { setShowNewFieldDialog(false); resetNewField(); }} size="sm" className="h-8">Cancel</Button>
                  <Button onClick={handleCreateField} size="sm" className="h-8">Create Field</Button>
                </div>
              </motion.div>
            </div>
          </>
              ...
              >
                <div className="p-4 border-b sticky top-0 bg-white z-10">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setShowFieldPropertiesDialog(false)} className="h-8 w-8 p-0 flex-shrink-0">
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
                ...
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <Label className="text-xs mb-1 block">Field Label</Label>
                      <Input value={editingField.field.label} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, label: e.target.value } })} className="h-8 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs mb-1 block">Width</Label>
                      <select value={editingField.field.width} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, width: e.target.value as InlineFieldDefinition['width'] } })} className="w-full rounded-md border border-input bg-background px-2.5 py-1.5 text-sm h-8">
                        <option value="full">Full Width</option>
                        <option value="half">Half Width (50%)</option>
                        <option value="third">Third Width (33%)</option>
                        <option value="two-thirds">Two Thirds (66%)</option>
                  ...
                  <div>
                    <Label className="text-xs mb-1 block">Placeholder</Label>
                    <Input value={editingField.field.placeholder || ''} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, placeholder: e.target.value } })} className="h-8 text-sm" />
                  </div>

                  <div>
                    <Label className="text-xs mb-1 block">Help Text</Label>
                    <Textarea value={editingField.field.help_text || ''} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, help_text: e.target.value } })} rows={2} className="text-sm resize-none" />
                  </div>
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setEditingField({ ...editingField, field: { ...editingField.field, options: [...currentOptions, { value: `option_${Date.now()}`, label: 'New Option' }] } });
                          }}
                          className="h-7 text-xs"
                        >
                          <Plus className="h-3 w-3 mr-1" />
                        ...
                        {(editingField.field.options as Array<{ value: string; label: string }>).map((option: { value: string; label: string }, index: number) => (
                          <div key={index} className="flex gap-1.5">
                            <Input
                              value={option.label}
                              onChange={(e) => {
                                const opts = editingField.field.options as Array<{ value: string; label: string }>;
                                const newOpts = [...opts];
                                newOpts[index] = { ...newOpts[index], label: e.target.value };
                                setEditingField({ ...editingField, field: { ...editingField.field, options: newOpts } });
                              }}
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                const opts = editingField.field.options as Array<{ value: string; label: string }>;
                                const newOpts = [...opts];
                                newOpts[index] = { ...newOpts[index], value: e.target.value };
                                setEditingField({ ...editingField, field: { ...editingField.field, options: newOpts } });
                              }}
                              placeholder="Value"
                              className="h-8 text-sm flex-1"
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={editingField.field.is_required} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, is_required: e.target.checked } })} className="rounded border-gray-300 h-3.5 w-3.5" />
                      <span className="text-xs">Required</span>
                    </label>
                    <label className="flex items-center space-x-1.5 cursor-pointer">
                      <input type="checkbox" checked={editingField.field.readonly} onChange={(e) => setEditingField({ ...editingField, field: { ...editingField.field, readonly: e.target.checked } })} className="rounded border-gray-300 h-3.5 w-3.5" />
                      <span className="text-xs">Read-only</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="p-3 border-t flex justify-end gap-2 sticky bottom-0 bg-white">
                  <Button variant="outline" onClick={() => { setShowFieldPropertiesDialog(false); setEditingField(null); }} size="sm" className="h-8">Cancel</Button>
                  <Button onClick={handleSaveFieldProperties} size="sm" className="h-8">Save Changes</Button>
                </div>
              </motion.div>
            </div>
          </>
