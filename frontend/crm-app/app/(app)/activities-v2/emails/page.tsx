...
function getInitialsFromSubject(subject?: string | null): string {
  if (!subject?.trim()) return "E";
  const words = subject.trim().split(/\s+/);
  if (words.length >= 2) return `${words[0][0]}${words[1][0]}`.toUpperCase();
  return subject.slice(0, 2).toUpperCase();
}

export default function EmailsV2Page() {
  ...
  const emailsFilters = (filters as Record<string, Record<string, unknown>>)["emails-v2"] || {};
  const [searchQuery, setSearchQuery] = useState<string>((emailsFilters.search as string) || "");
  const [statusFilter, setStatusFilter] = useState<string | null>((emailsFilters.status as string) || null);
  ...
  const { presets, addPreset, deletePreset } = useFilterPresets("emails-v2");
  ...
  const { data: statsData } = useActivitiesV2Stats("email");
  ...
  useEffect(() => {
    (setModuleFilters as (m: string, f: Record<string, unknown>) => void)("emails-v2", { search: searchQuery, status: statusFilter });
  ...
  const filterOptions = useMemo(() => {
    ...
    return [
      { label: "All Emails", value: null, count: total },
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try { await deleteActivity.mutateAsync(itemToDelete.id); toast.success("Email deleted"); setIsDeleteModalOpen(false); setItemToDelete(null); }
  ...
  const handleBulkExport = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({ activity_type: "email", ids: selectedItems.join(",") });
      ...
      toast.success(`Exported ${selectedItems.length} emails`);
    } catch {
  ...
  const handleEditItem = useCallback((item: ActivityV2) => {
    setFormMode("edit");
    const full = emails.find(e => e.id === item.id);
  ...
  const handleFormSubmit = async (data: CreateActivityV2Input) => {
    try {
      ...
      toast.success(formMode === "edit" ? "Email updated" : "Email created");
    } catch { throw new Error("Failed to save email"); }
  };
  ...
  const handleClearAllFilters = () => { setStatusFilter(null); setSearchQuery(""); setFilterGroup(null); (clearModuleFilters as (m: string) => void)("emails-v2"); };
  ...
  const columns = useMemo(() => [
    ...
    {
      key: "email_direction", label: "Direction",
    ...
    { key: "status", label: "Status", render: (v: string) => <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(v)}`}>{STATUS_DISPLAY[v] || v}</span> },
    {
      ...
      render: (_v: unknown, row: ActivityV2) => (
        <div className="text-sm text-muted-foreground">{row.display_contact || row.display_company || "—"}</div>
      ),
    },
    { key: "created_at", label: "Created", render: (v: string) => <div className="flex items-center gap-2 text-sm text-muted-foreground"><Calendar className="h-4 w-4" />{formatDate(v)}</div> },
  ], [handleEditItem]);
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <>
            ...
            <div className="relative" ref={filterDropdownRef}>
              <Button variant="outline" size="sm" onClick={() => setShowFilterDropdown(!showFilterDropdown)}>
                <Filter className="h-4 w-4 mr-2" />{statusFilter ? filterOptions.find(f => f.value === statusFilter)?.label : "All Emails"}<ChevronDown className="h-4 w-4 ml-2" />
              </Button>
              <AnimatePresence>
                {showFilterDropdown && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {emails.map((item, index) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <Card className="border border-border hover:shadow-lg transition-shadow p-6 cursor-pointer" onClick={() => handleEditItem(item)}>
                ...
                <div className="mt-4 flex items-center gap-2">
                  {item.email_direction && <span className={`px-3 py-1 rounded-full text-xs font-medium ${getDirectionColor(item.email_direction)}`}>{EMAIL_DIRECTION_DISPLAY[item.email_direction] || item.email_direction}</span>}
      ...
      <BulkDeleteModal isOpen={showBulkDelete} onClose={() => setShowBulkDelete(false)} onConfirm={handleBulkDelete} itemCount={selectedItems.length} itemName="email" />

      <BulkUpdateModal<"pending" | "in_progress" | "completed" | "cancelled">
        isOpen={showBulkUpdateStatus} onClose={() => setShowBulkUpdateStatus(false)} onConfirm={handleBulkUpdateStatus} itemCount={selectedItems.length} title="Update Email Status" field="Status"
