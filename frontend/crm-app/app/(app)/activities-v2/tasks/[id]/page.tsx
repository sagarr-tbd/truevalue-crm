...
export default function TaskV2DetailPage() {
  ...
  const { data: task, isLoading, isError } = useActivityV2(id);
  ...
  const handleDeleteConfirm = async () => {
    if (!task?.id) return;
    try {
      await deleteActivityV2.mutateAsync(task.id);
      router.push("/activities-v2/tasks");
    } catch {
  ...
  const handleToggleComplete = async () => {
    ...
    if (task.status === "completed") {
      await updateActivityV2.mutateAsync({ id: task.id, data: { status: "pending" } });
    } else {
      await completeActivityV2.mutateAsync(task.id);
    }
  };
  ...
  const handleFormSubmit = async (data: CreateActivityV2Input) => {
    if (!task?.id) return;
    try {
      await updateActivityV2.mutateAsync({ id: task.id, data });
  ...
  if (isError || !task) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <Button onClick={() => router.push("/activities-v2/tasks")}>
  ...
  const initials = task.subject ? task.subject.substring(0, 2).toUpperCase() : "T";
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push("/activities-v2/tasks")}
              className="gap-2"
            >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">Created {formatDate(task.created_at)}</span>
                </div>
              </div>
            </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {daysRemaining !== null && (
                <p
                  className={`text-xs ${
                    daysRemaining < 0 ? "text-destructive" : daysRemaining === 0 ? "text-orange-600" : "text-muted-foreground"
                  }`}
                >
                  {daysRemaining < 0
          ...
          <div className="lg:col-span-3">
            <Card className="mb-6">
              ...
              <CardContent className="space-y-6">
                {/* Task Information & Schedule & Timing */}
                    ...
                    <div className="space-y-3">
                      ...
                      {task.start_time && (
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-sm font-medium">{formatDateTime(task.start_time)}</span>
                        </div>
                      )}
                      {task.end_time && (
                        <div className="flex justify-between items-start">
                          <span className="text-sm text-muted-foreground">End Time</span>
                          <span className="text-sm font-medium">{formatDateTime(task.end_time)}</span>
                        </div>
                      )}
                      {task.duration_minutes != null && (
                        <div className="flex justify-between items-start">
                          <span className="text-sm text-muted-foreground">Duration</span>
                          <span className="text-sm font-medium">{task.duration_minutes} min</span>
                        </div>
                      )}
                      {task.reminder_at && (
                        <div className="flex justify-between items-start">
                          <span className="text-sm text-muted-foreground">Reminder</span>
                          <span className="text-sm font-medium">{formatDateTime(task.reminder_at)}</span>
                        </div>
                      )}
                      {task.completed_at && (
                        <div className="flex justify-between items-start">
                          <span className="text-sm text-muted-foreground">Completed At</span>
                          <span className="text-sm font-medium">{formatDateTime(task.completed_at)}</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                {can(TASKS_WRITE) && (
                  <Button className="w-full justify-start gap-2" variant="outline" onClick={handleEditTask}>
                    <Edit className="h-4 w-4" />
                    Edit Task
                  </Button>
                )}
            ...
            {/* Related Items */}
            {(task.display_contact || task.display_company || task.display_deal || task.display_lead) && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Related Items</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {task.display_contact && task.contact_id && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Contact</p>
                      <Link
                        href={`/sales-v2/contacts/${task.contact_id}`}
                      ...
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-teal to-brand-purple text-white flex items-center justify-center text-xs font-semibold">
                          {task.display_contact.substring(0, 2).toUpperCase()}
                        </div>
                        <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                          {task.display_contact}
                        </p>
                  ...
                  {task.display_company && task.company_id && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Company</p>
                      <Link
                        href={`/sales-v2/companies/${task.company_id}`}
                        className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group"
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-teal to-brand-purple text-white flex items-center justify-center text-xs font-semibold">
                          {task.display_company.substring(0, 2).toUpperCase()}
                        </div>
                        <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                          {task.display_company}
                        </p>
                        <LinkIcon className="h-3 w-3 text-muted-foreground ml-auto" />
                      </Link>
                    </div>
                  ...
                  {task.display_deal && task.deal_id && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Deal</p>
                      <Link
                        href={`/sales-v2/deals/${task.deal_id}`}
                        className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group"
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-teal to-brand-purple text-white flex items-center justify-center text-xs font-semibold">
                          {task.display_deal.substring(0, 2).toUpperCase()}
                        </div>
                        <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                          {task.display_deal}
                        </p>
                  ...
                  {task.display_lead && task.lead_id && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Lead</p>
                      <Link
                        href={`/sales-v2/leads/${task.lead_id}`}
                        className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group"
                      >
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-teal to-brand-purple text-white flex items-center justify-center text-xs font-semibold">
                          {task.display_lead.substring(0, 2).toUpperCase()}
                        </div>
                        <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                          {task.display_lead}
                        </p>
