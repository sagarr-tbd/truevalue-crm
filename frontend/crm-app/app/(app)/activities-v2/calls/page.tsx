...
export default function CallsV2Page() {
  ...
  const callsV2Filters = (filters as Record<string, Record<string, unknown>>)["calls-v2"] || {};

  const [searchQuery, setSearchQuery] = useState<string>(
    (callsV2Filters.search as string) || ""
  );
  const [statusFilter, setStatusFilter] = useState<string | null>(
    (callsV2Filters.status as string) || null
  );
  ...
  const { presets, addPreset, deletePreset } = useFilterPresets("calls-v2");
  ...
  // Build query params - filter by activity_type: "call"
  ...
  const { data: statsData } = useActivitiesV2Stats("call");
  ...
  const [callToDelete, setCallToDelete] = useState<ActivityV2 | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  const [formDrawerOpen, setFormDrawerOpen] = useState(false);
  const [editingCall, setEditingCall] = useState<ActivityV2 | null>(null);
  ...
  const filterOptions = useMemo(() => {
    ...
    return [
      { label: "All Calls", value: null, count: total },
  ...
  const stats = useMemo(() => {
    const totalCalls = statsData?.total ?? totalItems;
  ...
  const exportParams = useMemo(() => {
    const p: Record<string, string> = { activity_type: "call" };
  ...
  const handleDeleteClick = (call: ActivityV2) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteActivityV2.mutateAsync(callToDelete.id);
  ...
  const handleEditCall = useCallback((call: ActivityV2) => {
    setFormMode("edit");
    const fullCall = calls.find((c) => c.id === call.id);
    if (fullCall) {
      setEditingCall(fullCall);
      setFormDrawerOpen(true);
    }
  }, [calls]);
  ...
  const handleFormSubmit = async (data: CreateActivityV2Input) => {
    try {
      if (formMode === "edit" && editingCall?.id) {
        await updateActivityV2.mutateAsync({ id: editingCall.id, data });
      } else {
      ...
      setEditingCall(null);
      toast.success(formMode === "edit" ? "Call updated" : "Call created");
    } catch {
  ...
  const handleClearAllFilters = () => {
    ...
    setSearchQuery("");
    setFilterGroup(null);
    (clearModuleFilters as (module: string) => void)("calls-v2");
  };
    ...
    () => [
      {
        ...
        render: (_value: unknown, row: ActivityV2) => (
          <div
            ...
            onClick={() => router.push(`/activities-v2/calls/${row.id}`)}
          >
      ...
      },
      {
        ...
        render: (value: string) =>
          value ? (
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${getDirectionColor(value)}`}
            >
              {CALL_DIRECTION_DISPLAY[value] || value}
            </span>
          ) : (
      ...
      },
      {
        ...
        render: (value: string) =>
          value ? (
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${getOutcomeColor(value)}`}
            >
              {CALL_OUTCOME_DISPLAY[value] || value}
            </span>
          ) : (
            <span className="text-muted-foreground/50">—</span>
          ),
  ...
  const actionMenuItems = useCallback(
    (call: ActivityV2) => [
      {
        label: "View Details",
        icon: FileText,
        onClick: () => router.push(`/activities-v2/calls/${call.id}`),
      },
    ...
    [router, can, handleEditCall, completeActivityV2]
  );
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-brand-teal to-brand-purple text-white flex items-center justify-center text-sm font-semibold">
                      {getInitialsFromSubject(call.subject)}
                    </div>
                    <div>
                  ...
                  <div onClick={(e) => e.stopPropagation()}>
                    <ActionMenu items={actionMenuItems(call)} />
                  </div>
                </div>
                {call.description && (
                ...
                <div className="space-y-2">
                  ...
                  {(call.display_contact || call.display_company) && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Flag className="h-4 w-4" />
                      <span>{call.display_contact || call.display_company}</span>
                    </div>
                  )}
                </div>
                <div className="mt-4 flex items-center gap-2 flex-wrap">
                  ...
                  {call.call_direction && (
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getDirectionColor(call.call_direction)}`}
                    >
                  ...
                  {call.call_outcome && (
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getOutcomeColor(call.call_outcome)}`}
                    >
